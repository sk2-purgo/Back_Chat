...
## 프로젝트 소개
이 프로젝트는 비속어 사용을 줄이고 건강한 커뮤니티 문화를 조성하기 위한 PURGO 체험용 채팅 백엔드 시스템입니다.  
WebSocket 기반 실시간 채팅과 함께, AI 필터링 서버(PURGO_CORE)를 통해  
비속어가 포함된 메시지를 자동으로 순화하며, 감지 시마다 사용자의 사용 횟수를 누적 관리합니다.  
...
- **실시간 채팅 (WebSocket 기반)**  
  WebSocket과 STOMP 프로토콜을 사용해 클라이언트 간 실시간 메시지를 송수신합니다.

- **욕설 필터링 (FastAPI 연동)**  
  채팅 메시지는 AI 기반 욕설 필터링 서버로 전달되며, 감지 시 자동으로 대체어로 변환됩니다.

- **채팅방 입퇴장 추적**  
  사용자의 입장·퇴장 상태를 실시간으로 감지하고 기록하며, 모든 사용자가 퇴장하면 채팅방을 초기화합니다.

- **JPA 기반 DB 연동**  
  채팅방(ChatRoom), 메시지(Message) 등 모든 데이터는 JPA를 통해 MySQL에 자동 저장됩니다.

- **비속어 사용 횟수 누적 관리**  
  채팅방에서 사용된 모든 욕설을 감지하여 누적하고, 로그로 기록해 관리합니다.
